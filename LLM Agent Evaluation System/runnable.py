...
from langgraph.checkpoint.aiosqlite import AsyncSqliteSaver  # type: ignore # type: ignore
...
from tools.asana import available_asana_functions
from tools.google_drive import available_drive_functions
from tools.vector_db import available_vector_db_functions
...
def get_local_model():
    return HuggingFaceEndpoint(
        repo_id=model, task="text-generation", max_new_tokens=1024, do_sample=False
    )
...
for key, chatbot_class in model_mapping.items():
    if key in model.lower():
        chatbot = (
            chatbot_class(model=model)
            if key != "huggingface"
            else chatbot_class(llm=get_local_model())
        )
        break
...
async def call_model(
    ...
    messages = list(
        filter(
            lambda m: not isinstance(m, AIMessage)
            or hasattr(m, "response_metadata")
