...
def run_sql_select_statement(sql_statement):
    ...
    # Add header
    header = " | ".join(
        name.ljust(width) for name, width in zip(column_names, col_widths)
    )
    result_str += header + "\n"
    ...
    for row in records:
        row_str = " | ".join(
            str(value).ljust(width) for value, width in zip(row, col_widths)
        )
        result_str += row_str + "\n"
...
sql_router_agent = Agent(
    name="Router Agent", instructions=get_sql_router_agent_instructions()
)
...
sql_router_agent.functions = [
    transfer_to_rss_feeds_agent,
    transfer_to_user_agent,
